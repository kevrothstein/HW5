...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/kevinmrHW5"
## Provided:
...
class TodoListForm(FlaskForm):
    ...
    items = TextAreaField("Enter your TODO list items in the following format: Description, Priority -- separated by a comma and on the same line")
...
def update(item):
    ...
    if form.validate_onsubmit():
        item_d = TodoItem.query.filter_by(id=int(item)).first()
        kev = form.new_priority.data
        item_d.priority = kev
        db.session.commit()
        flash('Update priority of {}'.format(item))
        return redirect(url_for('all_lists'))
